...
void Game::Update(DX::StepTimer const& timer)
{
	...
	{
		m_camOrientation.y += (float)deltaMouseX/2.0f;
		m_camOrientation.z += (float)deltaMouseY/2.0f;
	...
	if (currentSelection != oldSelection)
	{
		...
		{
			//make our selection wireframe
			m_displayList[currentSelection].m_wireframe = true;
		}

		if (oldSelection != -1)
		{
			//make our old selection opaque
			m_displayList[oldSelection].m_wireframe = false;
		}
	}
	...
	case X:
		if (currentSelection != -1)
		{
			...
			{
				//transform along the X axis (also follows the terrain Y)
				WorldMousePosition();
				m_displayList[currentSelection].m_position.x = worldMouseX;
				m_displayList[currentSelection].m_position.y = worldMouseY;
			}
	...
	case Y:
		if (currentSelection != -1)
		{
			if (m_InputCommands.leftClick)
			{
				//move up and down according to the mouse Y axis
				m_displayList[currentSelection].m_position.y -= (float)deltaMouseX / 10;
			}
		}
		break;
	...
	case Z:
		if (currentSelection != -1)
		{
			if (m_InputCommands.leftClick)
			{
				//Transform to the world mouse Z
				WorldMousePosition();
	...
	case XZ:
		if (currentSelection != -1)
		{
			if (m_InputCommands.leftClick)
			{
				//transform to the world mouse X and Z
				WorldMousePosition();
				m_displayList[currentSelection].m_position.x = worldMouseX;
				m_displayList[currentSelection].m_position.z = worldMouseZ;
				m_displayList[currentSelection].m_position.y = worldMouseY;
	...
	case Scale:
	{
		if (currentSelection != -1)
		{
			if (m_InputCommands.leftClick)
			{
				//clamp to make sure it doesn't go negative
				if (m_displayList[currentSelection].m_scale.x > 0.1f)
				{					
					m_displayList[currentSelection].m_scale -= Vector3((float)deltaMouseX / 10, (float)deltaMouseX / 10, (float)deltaMouseX / 10);
				}
				else
				{
					//reset the scale if it goes out of bounds
					m_displayList[currentSelection].m_scale = Vector3(0.15f, 0.15f, 0.15f);
				}
			}
		}
		break;
...
{
	//XNA Picking Tutorial used for this, code is modified for DirectX but the methodology is still the same.
	int pickedObjectID = -1;
	float minDistancer = std::numeric_limits<float>::max();

	//setup our near and far planes
	...
	{
		//create a matrix of the relevant object
		...
		//create set the matrix of the selected object in the world

		XMMATRIX local = m_world * XMMatrixTransformation(g_XMZero, Quaternion::Identity, scale, g_XMZero, rotate, translate);

		//vectors of calculating far and near points
		...
		for (int y = 0; y < m_displayList[i].m_model.get()->meshes.size(); y++)
		{
			...
			if (m_displayList[i].m_model.get()->meshes[y]->boundingBox.Intersects(nearPoint, direction, distance))
			{
				...
				{
					//this is the object we picked, don't overwrite it with anything behind it.
	...
	if (pickedObjectID > -1)
	{
		//set picked to the currently selected
		currentSelection = pickedObjectID;
	}
}

void Game::ToggleTerrainWireframe()
{
	//toggle wireframe if it's off
	if (!terrainWireframe)
	{
...
void Game::WorldMousePosition()
{
	...
	//And this is where I would put my quadtree... IF I HAD ONE (or knew how to make one...)


	//create a local matrix for the point of interest
	XMMATRIX local;

	//and make it the identity
	local = XMMatrixIdentity();
			
	//calculate far and near points of our ray (this is what's killing the framerate STONE dead)
	XMVECTOR nearPoint = XMVector3Unproject(nearSource, 0.0f, 0.0f, clientRect.right, clientRect.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_view, local);
	XMVECTOR farPoint = XMVector3Unproject(farSource, 0.0f, 0.0f, clientRect.right, clientRect.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_view, local);
	...
	for (int i = 0; i < m_displayChunk.GetTerrainResolution(); i++)
	{
		...
		{
				//get the position of the terrain point
				DirectX::XMFLOAT3 terrainPosition = m_displayChunk.GetVertexPositions(i, j);
				//get the center point of the terrain tile
				...
				//aaaand we now have our bounding box (which surprisingly isn't what's killing the framerate)
				...
				{
					//prevents our first selection from being overwriten by things behind it
					if (distance < minDistancer)
					{
						minDistancer = distance;

						//finally, pass the tile co-orinate to the world mouse co-ord
						worldMouseX = center.x;
						worldMouseY = terrainPosition.y;
						worldMouseZ = center.z;
					}
				}
		}
	}
...
void Game::Render()
{
	...
	std::wstring var3 = L"Delta Mouse X: " + std::to_wstring(deltaMouseX) + L"Delta Mouse Y: " + std::to_wstring(deltaMouseY);
	m_font->DrawString(m_sprites.get(), var.c_str(), XMFLOAT2(100, 10), Colors::Yellow);
	m_font->DrawString(m_sprites.get(), var2.c_str(), XMFLOAT2(100, 40), Colors::Yellow);
	m_font->DrawString(m_sprites.get(), var3.c_str(), XMFLOAT2(100, 80), Colors::Yellow);
	m_sprites->End();
	...
	//check if the terrain is supposed to be in wireframe
	if (terrainWireframe)
	{
		context->RSSetState(m_states->Wireframe());		//enable wireframe
	}
